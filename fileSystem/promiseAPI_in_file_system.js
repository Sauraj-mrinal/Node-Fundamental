...
// if path not match then  recursive: true-> will create path asper my given path  and then create folder 
try {
 await fs.mkdir('E:\\Node\\Node-Fundamental\\fileSystem\\newFileCreated\\demo', { recursive: true });
 console.log("Directory Created....");
} catch (error) {
 console.log(error);
}
...
// Copy File
try {
  fs.copyFile('readme.txt', 'E:\\Node\\Node-Fundamental\\info.txt');
...
try {
 const stats =  fs.stat('E:\\Node\\Node-Fundamental\\emp.js');
